...
class LanguageModel(object):
    ...
    def _build_word_embeddings(self):
        ...
        with tf.device("/cpu:0"):
            ### 使用glove初始化
            embedding_path = r'D:\work\daguan\data\w2v\wv.vector'
            w2v_model = gensim.models.KeyedVectors.load_word2vec_format(fname=embedding_path)
    ...
    with tf.Session(config=tf.ConfigProto(
        ...
        for batch_no, batch in enumerate(data_gen, start=1):
            ...
            if (batch_no % 2000 == 0) or (batch_no == n_batches_total):
                ...
                embedding_path = os.path.join(tf_save_dir, 'vocab_embedding.hdf5')
                with h5py.File(embedding_path, 'w') as fout:
